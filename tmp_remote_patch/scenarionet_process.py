...
_ORIGINAL_CREATE_AGENTS = VehicleAgentManager._create_agents


def _create_agents_with_vehicle_defaults(self, config_dict: dict):
  base_vehicle_cfg = dict(self.engine.global_config.get("vehicle_config", {}))
  patched_cfg = {}
  for agent_id, agent_cfg in config_dict.items():
    merged_cfg = dict(base_vehicle_cfg)
    merged_cfg.update(dict(agent_cfg))
    merged_cfg["render_vehicle"] = True
    patched_cfg[agent_id] = merged_cfg
  return _ORIGINAL_CREATE_AGENTS(self, patched_cfg)


if not getattr(VehicleAgentManager, "_openpilot_vehicle_defaults_patch", False):
  VehicleAgentManager._create_agents = _create_agents_with_vehicle_defaults
  VehicleAgentManager._openpilot_vehicle_defaults_patch = True
