...
class SimulatorBridge(ABC):
  ...
  def __init__(self, dual_camera, high_quality):
    ...
    self.last_reengage_attempt = 0.0
    self.reengage_interval = 1.0
...
Ignition: {self.simulator_state.ignition} Engaged: {self.simulator_state.is_engaged}
  ...
  def _run(self, q: Queue):
    ...
    while self._keep_alive:
      ...
      engageable = self.simulated_car.sm['selfdriveState'].engageable
      ...
      if self.simulator_state.is_engaged:
        ...
        # Force startup engagement and automatically re-engage if active drops mid-route.
        now = time.monotonic()
        should_reengage = (not self.past_startup_engaged) or ((now - self.last_reengage_attempt) >= self.reengage_interval)
        if should_reengage:
          ...
          self.last_reengage_attempt = now
